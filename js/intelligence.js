...
function renderRootCauseSelector(rows) {
  try {
    let list = rows;
    // If there are no delayed activities, fall back to offering all activities so user can choose
    if (!list || !list.length) {
      list = activities.slice(0, 200); // limit for performance
    }
    const options = ['<option value="">Select delayed activity</option>']
      .concat(list.map((row) => `<option value="${escapeHtml(row.activityId)}">${escapeHtml(row.activityId)} - ${escapeHtml(row.activityName || "-")}</option>`))
      .join("");
    if (dom.rootCauseActivity) dom.rootCauseActivity.innerHTML = options;
  } catch (err) {
    console.error("[intelligence] renderRootCauseSelector error", err);
  }
}
...
function renderRiskTable(rows) {
  if (!rows.length) {
    dom.riskTableBody.innerHTML = `<tr><td colspan="9"><div class="empty-state">No delayed or elevated-risk activities found.</div></td></tr>`;
    return;
  }
    ...
    if (dom.saveRootCauseButton) {
      dom.saveRootCauseButton.addEventListener("click", () => {
        try {
          ...
          notify("Failed to save root cause. See console for details.", "error");
        }
      });
    }
  } catch (err) {
