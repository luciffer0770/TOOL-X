...
/**
 * Lightweight predictive heuristic for delay and finish date.
 * Uses current risk score, material signal, and dependency complexity
 * to estimate additional delay hours beyond the current observed delay.
 */
function predictDelayAndFinish(activity, referenceDate = new Date()) {
  const plannedDuration = Math.max(1, getPlannedDurationHours(activity));
  const existingDelay = Math.max(0, getDelayHours(activity, referenceDate));

  // If already delayed or completed, take current delay as baseline.
  const status = String(activity.activityStatus || "").toLowerCase();
  if (status === "completed") {
    const plannedEnd = parseDate(activity.plannedEndDate);
    const actualEnd = parseDate(activity.actualEndDate);
    const finalDelay = existingDelay;
    return {
      predictedDelayHours: Math.round(finalDelay * 10) / 10,
      predictedFinishDate: dateToIso(actualEnd || plannedEnd || referenceDate),
    };
  }

  const riskScore = toNumber(activity.riskScore);

  // Derive simple probability of further delay (0–1) from risk bands.
  let baseProb = riskScore / 100;
  if (riskScore >= 75) baseProb = Math.max(baseProb, 0.85);
  else if (riskScore >= 55) baseProb = Math.max(baseProb, 0.65);
  else if (riskScore >= 30) baseProb = Math.max(baseProb, 0.4);

  // Material factor: high/critical and not received amplify delay risk.
  const materialStatus = String(activity.materialStatus || "").toLowerCase();
  const criticality = String(activity.materialCriticality || "").toLowerCase();
  ...
  // Dependency factor: more predecessors and non-FS types increase uncertainty.
  const dependencies = parseDependencies(activity.dependencies);
  const depCount = dependencies.length;
  let dependencyFactor = clamp(depCount * 0.03, 0, 0.25);
  const depType = String(activity.dependencyType || "").toUpperCase();
  if (depType === "SS" || depType === "FF") {
    dependencyFactor += 0.05;
  }

  const predictedProb = clamp(baseProb + materialFactor + dependencyFactor, 0, 1);

  // Expected additional delay scaled against planned duration.
  const expectedAdditionalDelay = clamp(predictedProb * plannedDuration * 0.4, 0, plannedDuration * 0.75);
  const totalPredictedDelay = existingDelay + expectedAdditionalDelay;

  const plannedEnd = parseDate(activity.plannedEndDate) || parseDate(activity.actualEndDate);
  const anchorDate =
    plannedEnd && plannedEnd > referenceDate
      ? plannedEnd
      : plannedEnd || referenceDate;
  const predictedFinishMs = anchorDate.getTime() + totalPredictedDelay * HOURS_TO_MS;
  const predictedFinishDate = new Date(predictedFinishMs);

  return {
    predictedDelayHours: Math.round(totalPredictedDelay * 10) / 10,
    predictedFinishDate: dateToIso(predictedFinishDate),
    predictedDelayProbability: predictedProb,
  };
}
...
export function enrichActivity(activity, referenceDate = new Date()) {
  ...
  const prediction = predictDelayAndFinish(
    {
      ...sanitized,
      activityStatus,
      riskScore,
  ...
  return {
    ...
    predictedFinishDate: prediction.predictedFinishDate,
    predictedDelayProbability: prediction.predictedDelayProbability,
  };
}
