...
function updateUndoButton() {
  ...
  if (dom.redoBtn) {
    const okr = canRedo();
    dom.redoBtn.style.display = okr ? "" : "none";
    dom.redoBtn.textContent = okr ? `Redo: ${getRedoDescription() || "last undone"}` : "Redo";
  }
}
...
function initialize() {
  ...
  initPage({
    ...
    onReady(user) {
      ...
      wireEvents();
      // Global shortcuts scoped to activities page
      registerShortcut({
        combo: "Ctrl+K",
        scope: "activities",
        handler: (event) => {
      ...
      registerShortcut({
        ...
        scope: "activities",
        handler: () => {
          if (canModifyActivityStructure(currentUser) && dom.addButton) {
            dom.addButton.click();
            return false;
          }
          return undefined;
      ...
      registerShortcut({
        ...
        scope: "activities",
        handler: () => {
          if (canImportExportData(currentUser) && dom.exportCsvButton) {
            dom.exportCsvButton.click();
            return false;
          }
          return undefined;
        },
